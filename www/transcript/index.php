<?
	...
	$transcriptDir = '/home/dropbox/Dropbox/linked_jazz_transcripts/';		//the location of the transcripts on the server
	$transcriptTextDir = '/home/data/transcript/data/';
	...
	if (isset($_REQUEST['json'])){
		...
		if (isset($_REQUEST['processTranscript'])){
			...
			if ($results==''){				
				...
				//run the 
				$results = shell_exec('python /home/data/transcript/ner.py "' . $filename . '.txt" 2>&1');
	...
	if (isset($_REQUEST['askWhy'])){
		...
		$results = shell_exec('grep "' . $_REQUEST['askWhy'] . '" /var/www/html/data/allJazz.nt');
		echo $results;
	...
	if (isset($_REQUEST['dbpedia'])){
		...
		$fh = fopen("/var/www/html/data/verified.nt", 'a') or die('error opening file');
	...
	if (isset($_REQUEST['deldbpedia'])){
		...
		$fh = fopen("/var/www/html/data/deleted.nt", 'a') or die('error opening file');
	...
	if (isset($_REQUEST['undeldbpedia'])){
		...
		$results = shell_exec('grep -v "' . $removeLine . '" /var/www/html/data/deleted.nt> /var/www/html/data/deleted.tmp.nt');
		$results = shell_exec('mv /var/www/html/data/deleted.tmp.nt /var/www/html/data/deleted.nt');
	...
	if (isset($_REQUEST['undbpedia'])){
		...
		$results = shell_exec('grep -v "' . $removeLine . '" /var/www/html/data/verified.nt > /var/www/html/data/verified.tmp.nt');
		$results = shell_exec('mv /var/www/html/data/verified.tmp.nt /var/www/html/data/verified.nt');
						...
						if 	(tr.allNamesObject[x].interviewee){
							...
							if (tr.allNamesObject[x].authority.search('dbpedia') != -1 || tr.allNamesObject[x].authority.search('linkedjazz') != -1){
								...
								if (imageNames.indexOf(imageName) != -1){
									...
									imageName = '../image/round/' + imageName;
